        ...
        while sent_count < target_qty:
            ...
            try:
                ...
                for item in data_list:
                    ...
                    # STRICT RULE: If thumbnail is missing, SKIP ITEM
                    if not v_thumb or not v_thumb.startswith('http'):
                        logger.info(f"Skipping (No Thumb): {v_url}")
                        continue
                    ...
                    if save_video(v_url, v_title, category):
                        try:
                            ...
                            # STRICT RULE: Send Photo ONLY. If fails, log error and SKIP.
                            try:
                                await bot.send_photo(chat_id, v_thumb, caption=caption, parse_mode="HTML")
                                sent_count += 1
                                logger.info(f"Sent Photo [{sent_count}/{target_qty}]: {v_title}")
                                
                                # RATE LIMIT: 3 seconds
                                await asyncio.sleep(3)
                                
                            except Exception as send_err:
                                # If sending photo fails (e.g. blocked image), we DO NOT fallback to text.
                                # We simply skip and continue.
                                logger.warning(f"Photo send failed (Skipping Item): {send_err}")
